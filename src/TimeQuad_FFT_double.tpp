...
TimeQuad_FFT<double, DataType>::TimeQuad_FFT(np_double ks, py::array_t<DataType, py::array::c_style> data,
    ...
    : n_prod(compute_n_prod(ks)), ks_shape(get_shape(ks)), l_kernel(compute_l_kernels(ks)),
      ...
      quads(Multi_array<double, 2>(n_prod, l_full, fftw_malloc, fftw_free)), dt(dt), l_fft(l_fft),
...
template <class DataType> uint TimeQuad_FFT<double, DataType>::compute_n_prod(np_double &np_array) {
...
template <class DataType>
Multi_array<double, 2, uint32_t> TimeQuad_FFT<double, DataType>::copy_ks(np_double &np_ks, uint n_prod) {
...
template <class DataType> uint TimeQuad_FFT<double, DataType>::compute_l_kernels(np_double &np_array) {
    py::buffer_info buffer = np_array.request();
    std::vector<ssize_t> shape = buffer.shape;
...
template <class DataType>
std::vector<ssize_t> TimeQuad_FFT<double, DataType>::get_shape(np_double &np_array) {
    return np_array.request().shape;
}

template <class DataType>
uint64_t TimeQuad_FFT<double, DataType>::compute_l_data(py::array_t<DataType, py::array::c_style> &data) {
    py::buffer_info buffer = data.request();
    auto shape = buffer.shape;
    return shape[buffer.ndim - 1];
}

template <class DataType> void TimeQuad_FFT<double, DataType>::prepare_kernels(np_double &np_ks) {
...
template <class DataType>
void TimeQuad_FFT<double, DataType>::execution_checks(np_double &ks,
...
template <class DataType>
void TimeQuad_FFT<double, DataType>::execute_py(np_double &ks,
...
template <class DataType>
void TimeQuad_FFT<double, DataType>::execute(Multi_array<DataType, 1, uint64_t> &data) {
...
template <class DataType> np_double TimeQuad_FFT<double, DataType>::get_quads() {
    ...
    return py::array_t<double, py::array::c_style>(shape_valid, // shape
