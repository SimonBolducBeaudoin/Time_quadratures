...
TimeQuad_FFT<double, DataType>::TimeQuad_FFT(np_double ks,
    ...
    : n_prod(compute_n_prod(ks)), ks_shape(get_shape(ks)), l_kernel(compute_l_kernels(ks)),
      ...
      ks_complex(
          Multi_array<complex_d, 2, uint32_t>(n_prod, (l_fft / 2 + 1), fftw_malloc, fftw_free)),
...
template <class DataType>
Multi_array<double, 2, uint32_t> TimeQuad_FFT<double, DataType>::copy_ks(np_double &np_ks,
                                                                         uint n_prod) {
...
template <class DataType>
uint64_t
TimeQuad_FFT<double, DataType>::compute_l_data(py::array_t<DataType, py::array::c_style> &data) {
