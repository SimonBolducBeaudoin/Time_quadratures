...
// {
// return
// std::chrono::time_point_cast<std::chrono::nanoseconds>(t1).time_since_epoch().count()
// -
// std::chrono::time_point_cast<std::chrono::nanoseconds>(t0).time_since_epoch().count();
// }

template <class FloatType = double, class DataType = int16_t> class TimeQuad_FFT {};

template <class DataType> class TimeQuad_FFT<double, DataType> {
  public:
    // Contructor
    TimeQuad_FFT(np_double ks, py::array_t<DataType, py::array::c_style> data, double dt, uint l_fft,
                 int n_threads);
    // Destructor
    ~TimeQuad_FFT();
...
template <class DataType> class TimeQuad_FFT<float, DataType> {
  public:
    // Contructor
    TimeQuad_FFT(np_double ks, py::array_t<DataType, py::array::c_style> data, double dt, uint l_fft,
                 int n_threads);
