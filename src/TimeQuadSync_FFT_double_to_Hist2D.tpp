...
TimeQuadSync_FFT_to_Hist2D<double, BinType, DataType>::TimeQuadSync_FFT_to_Hist2D(
    ...
    : n_prod(compute_n_prod(ks)), n_hist(std::max(n_prod / 2, (uint)1)), ks_shape(get_shape(ks)),
      ...
      l_invalid(l_kernel - 1), ks(copy_ks(ks, n_prod)), quads(Multi_array<double, 2>(n_prod, l_qs)),
...
template <class BinType, class DataType>
uint TimeQuadSync_FFT_to_Hist2D<double, BinType, DataType>::compute_n_prod(np_double &np_array) {
...
TimeQuadSync_FFT_to_Hist2D<double, BinType, DataType>::copy_ks(np_double &np_ks, uint n_prod) {
    ...
    Multi_array<double, 2, uint32_t> ks(new_ptr, n_prod, shape.back() /*shape[-1]*/,
