...
TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::TimeQuad_FFT_to_Hist2D(
    ...
    : n_prod(compute_n_prod(ks)), ks_shape(get_shape(ks)),
      ...
      ks(copy_ks(ks, n_prod)), quads(Multi_array<double, 2>(n_prod, l_qs)),
      ks_complex(Multi_array<complex_d, 2, uint32_t>(n_prod, (l_fft / 2 + 1))),
      gs(Multi_array<double, 2, uint32_t>(n_threads, 2 * (l_fft / 2 + 1),
      ...
      hs(Multi_array<complex_d, 3, uint32_t>(n_threads, n_prod, (l_fft / 2 + 1),
                                             fftw_malloc, fftw_free)),
      Hs(Multi_array<BinType, 3, uint32_t>(n_threads, n_prod, nofbins,
                                           fftw_malloc, fftw_free)) {
  checks();
  prepare_plans();
...
// CHECKS
template <class BinType, class DataType>
void TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::checks() {
...
template <class BinType, class DataType>
uint TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::compute_n_prod(
    np_double &np_array) {
...
TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::copy_ks(np_double &np_ks,
                                                           uint n_prod) {
...
template <class BinType, class DataType>
uint TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::compute_l_kernels(
    np_double &np_array) {
  py::buffer_info buffer = np_array.request();
  std::vector<ssize_t> shape = buffer.shape;
...
template <class BinType, class DataType>
std::vector<ssize_t>
TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::get_shape(
    np_double &np_array) {
...
template <class BinType, class DataType>
void TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::prepare_kernels(
    np_double &np_ks) {
...
template <class BinType, class DataType>
inline void TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::float_to_hist(
    double data, BinType *histogram, double max, double bin_width) {
  std::abs(data) >= max
      ? histogram[0]++
...
template <class BinType, class DataType>
py::array_t<double>
TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::abscisse_py(double max,
                                                               uint nofbins) {
