...
TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::TimeQuad_FFT_to_Hist2D(
    np_double ks, py::array_t<DataType, py::array::c_style> data, double dt, uint l_fft, uint nofbins,
    ...
    : n_prod(compute_n_prod(ks)), n_hist(std::max(n_prod / 2, (uint)1)), ks_shape(get_shape(ks)),
      ...
      l_fft(l_fft), nofbins(nofbins), max(max), bin_width(2.0 * max / (nofbins)), n_threads(n_threads),
      ...
      l_qs_chunk(l_kernel - 1), ks(copy_ks(ks, n_prod)), quads(Multi_array<double, 2>(n_prod, l_qs)),
...
// CHECKS
template <class BinType, class DataType> void TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::checks() {
...
template <class BinType, class DataType>
void TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::prepare_plans() {
...
template <class BinType, class DataType>
void TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::destroy_plans() {
...
template <class BinType, class DataType>
uint TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::compute_n_prod(np_double &np_array) {
...
template <class BinType, class DataType>
Multi_array<double, 2, uint32_t> TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::copy_ks(np_double &np_ks,
                                                                                            uint n_prod) {
...
template <class BinType, class DataType>
uint TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::compute_l_kernels(np_double &np_array) {
    py::buffer_info buffer = np_array.request();
    std::vector<ssize_t> shape = buffer.shape;
...
template <class BinType, class DataType>
std::vector<ssize_t> TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::get_shape(np_double &np_array) {
    return np_array.request().shape;
}

template <class BinType, class DataType>
uint64_t TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::compute_l_data(
...
template <class BinType, class DataType>
void TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::prepare_kernels(np_double &np_ks) {
...
template <class BinType, class DataType>
void TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::execution_checks(
...
template <class BinType, class DataType>
void TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::execute_py(
...
template <class BinType, class DataType>
void TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::execute(Multi_array<DataType, 1, uint64_t> &data) {
...
#pragma omp for simd
        for (uint j = 0; j < n_prod; j += 2) {
            ...
            Hs.accumulate(data_1, data_2, (n_chunks - 1) * l_qs_chunk, j / 2, this_thread);
        }
    }
    Hs.reduction();
}

template <class BinType, class DataType> void TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::reset() {
...
template <class BinType, class DataType>
py::array_t<double> TimeQuad_FFT_to_Hist2D<double, BinType, DataType>::abscisse_py(double max, uint nofbins) {
