...
// CHECKS
template <class BinType, class DataType>
void TimeQuadSync_FFT_to_Hist2D<float, BinType, DataType>::checks() {
...
template <class BinType, class DataType>
void TimeQuadSync_FFT_to_Hist2D<float, BinType, DataType>::prepare_plans() {
...
template <class BinType, class DataType>
void TimeQuadSync_FFT_to_Hist2D<float, BinType, DataType>::destroy_plans() {
...
template <class BinType, class DataType>
uint TimeQuadSync_FFT_to_Hist2D<float, BinType, DataType>::compute_l_kernels(np_double &np_array) {
    py::buffer_info buffer = np_array.request();
    std::vector<ssize_t> shape = buffer.shape;
...
template <class BinType, class DataType>
std::vector<ssize_t> TimeQuadSync_FFT_to_Hist2D<float, BinType, DataType>::get_shape(np_double &np_array) {
    return np_array.request().shape;
}

template <class BinType, class DataType>
uint64_t TimeQuadSync_FFT_to_Hist2D<float, BinType, DataType>::compute_l_data(
        ...
        for (uint i = 0; i < n_chunks; i++) {
            ...
            for (uint j = 0; j < n_prod; j++) {
                for (uint k = 0; k < l_invalid; k++) {
					...
					#pragma omp atomic update
                    quads(j, (i + 1) * l_invalid + k) += ((float *)hs(this_thread, j))[l_chunk + k];
                }
            }
        }
		#pragma omp single
...
template <class BinType, class DataType>
py::array_t<double> TimeQuadSync_FFT_to_Hist2D<float, BinType, DataType>::abscisse_py(double max,
                                                                                       uint nofbins) {
