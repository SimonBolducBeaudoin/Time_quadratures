...
TimeQuad_FFT<float, DataType>::copy_ks(np_double &np_ks, uint n_prod) {
...
template <class DataType>
std::vector<ssize_t>
TimeQuad_FFT<float, DataType>::get_shape(np_double &np_array) {
  return np_array.request().shape;
}

template <class DataType>
uint64_t TimeQuad_FFT<float, DataType>::compute_l_data(
...
template <class DataType>
void TimeQuad_FFT<float, DataType>::execute_py(
...
template <class DataType>
void TimeQuad_FFT<float, DataType>::execute(
    Multi_array<DataType, 1, uint64_t> &data) {
