...
TimeQuad_FFT<float, DataType>::TimeQuad_FFT(np_double ks, py::array_t<DataType, py::array::c_style> data,
...
template <class DataType> uint TimeQuad_FFT<float, DataType>::compute_l_kernels(np_double &np_array) {
    py::buffer_info buffer = np_array.request();
    std::vector<ssize_t> shape = buffer.shape;
    return shape[buffer.ndim - 1];
}

template <class DataType> std::vector<ssize_t> TimeQuad_FFT<float, DataType>::get_shape(np_double &np_array) {
...
template <class DataType>
void TimeQuad_FFT<float, DataType>::execution_checks(np_double &ks,
