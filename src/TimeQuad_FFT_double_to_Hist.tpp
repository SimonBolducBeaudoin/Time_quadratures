...
TimeQuad_FFT_to_Hist<double, BinType, DataType>::TimeQuad_FFT_to_Hist(
    ...
    : n_prod(compute_n_prod(ks)), ks_shape(get_shape(ks)), l_kernel(compute_l_kernels(ks)),
      ...
      bin_width(2.0 * max / (nofbins)), n_threads(n_threads), l_chunk(compute_l_chunk(l_kernel, l_fft)),
      n_chunks(compute_n_chunks(l_data, l_chunk)), l_reste(compute_l_reste(l_data, l_chunk)),
      l_qs(compute_l_qs(l_kernel, n_chunks)), l_qs_chunk(l_kernel - 1), ks(copy_ks(ks, n_prod)),
...
// CHECKS
template <class BinType, class DataType> void TimeQuad_FFT_to_Hist<double, BinType, DataType>::checks() {
...
template <class BinType, class DataType>
Multi_array<double, 2, uint32_t> TimeQuad_FFT_to_Hist<double, BinType, DataType>::copy_ks(np_double &np_ks,
                                                                                          uint n_prod) {
...
template <class BinType, class DataType>
inline void TimeQuad_FFT_to_Hist<double, BinType, DataType>::float_to_hist(double data, BinType *histogram,
                                                                           double max, double bin_width) {
    std::abs(data) >= max ? histogram[0]++ : histogram[(unsigned int)((data + max) / (bin_width))]++;
}

template <class BinType, class DataType> void TimeQuad_FFT_to_Hist<double, BinType, DataType>::reset() {
