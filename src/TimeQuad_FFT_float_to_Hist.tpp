...
TimeQuad_FFT_to_Hist<float, BinType, DataType>::TimeQuad_FFT_to_Hist(
    np_double ks, py::array_t<DataType, py::array::c_style> data, float dt, uint l_fft,
    ...
    : n_prod(compute_n_prod(ks)), ks_shape(get_shape(ks)), l_kernel(compute_l_kernels(ks)),
      ...
      hs(Multi_array<complex_f, 3, uint32_t>((uint)n_threads, n_prod, (l_fft / 2 + 1), fftwf_malloc,
                                             fftwf_free)),
      Hs(Multi_array<BinType, 3, uint32_t>((uint)n_threads, n_prod, nofbins, fftwf_malloc,
                                           fftwf_free)) {
    checks();
    prepare_plans();
...
TimeQuad_FFT_to_Hist<float, BinType, DataType>::copy_ks(np_double &np_ks, uint n_prod) {
...
template <class BinType, class DataType>
inline void
TimeQuad_FFT_to_Hist<float, BinType, DataType>::float_to_hist(float data, BinType *histogram,
...
template <class BinType, class DataType>
py::array_t<double> TimeQuad_FFT_to_Hist<float, BinType, DataType>::abscisse_py(double max,
                                                                                uint nofbins) {
