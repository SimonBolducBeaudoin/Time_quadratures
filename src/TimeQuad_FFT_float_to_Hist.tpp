...
TimeQuad_FFT_to_Hist<float, BinType, DataType>::TimeQuad_FFT_to_Hist(
    np_double ks, py::array_t<DataType, py::array::c_style> data, float dt, uint l_fft, uint nofbins,
...
// CHECKS
template <class BinType, class DataType> void TimeQuad_FFT_to_Hist<float, BinType, DataType>::checks() {
...
void TimeQuad_FFT_to_Hist<float, BinType, DataType>::prepare_plans() {
    ...
    kernel_plan = fftwf_plan_dft_r2c_1d(l_fft, (float *)ks_complex(0),
...
template <class BinType, class DataType>
Multi_array<float, 2, uint32_t> TimeQuad_FFT_to_Hist<float, BinType, DataType>::copy_ks(np_double &np_ks,
                                                                                        uint n_prod) {
...
template <class BinType, class DataType>
inline void TimeQuad_FFT_to_Hist<float, BinType, DataType>::float_to_hist(float data, BinType *histogram,
                                                                          double max, double bin_width) {
    std::abs(data) >= max ? histogram[0]++ : histogram[(unsigned int)((data + max) / (bin_width))]++;
}

template <class BinType, class DataType> void TimeQuad_FFT_to_Hist<float, BinType, DataType>::reset() {
