...
TimeQuad_FFT_to_Hist<float, BinType, DataType>::TimeQuad_FFT_to_Hist(
    np_double ks, py::array_t<DataType, py::array::c_style> data, float dt,
    ...
    : n_prod(compute_n_prod(ks)), ks_shape(get_shape(ks)),
      ...
      l_full(compute_l_full(l_kernel, l_data)), dt(dt), l_fft(l_fft),
      nofbins(nofbins), max(max), bin_width(2.0 * max / (nofbins)),
      ...
      gs(Multi_array<float, 2, uint32_t>((uint)n_threads, l_fft, fftwf_malloc,
                                         fftwf_free)),
      fs(Multi_array<complex_f, 2, uint32_t>((uint)n_threads, (l_fft / 2 + 1),
                                             fftwf_malloc, fftwf_free)),
      hs(Multi_array<complex_f, 3, uint32_t>(
          (uint)n_threads, n_prod, (l_fft / 2 + 1), fftwf_malloc, fftwf_free)),
      Hs(Multi_array<BinType, 3, uint32_t>((uint)n_threads, n_prod, nofbins,
                                           fftwf_malloc, fftwf_free)) {
  checks();
  prepare_plans();
...
void TimeQuad_FFT_to_Hist<float, BinType, DataType>::prepare_plans() {
  ...
  kernel_plan = fftwf_plan_dft_r2c_1d(
      l_fft, (float *)ks_complex(0),
      reinterpret_cast<fftwf_complex *>(ks_complex(0)), FFTW_EXHAUSTIVE);
  g_plan = fftwf_plan_dft_r2c_1d(
      l_fft, gs[0], reinterpret_cast<fftwf_complex *>(fs[0]), FFTW_EXHAUSTIVE);
  int n[] = {(int)l_fft};
...
template <class BinType, class DataType>
uint TimeQuad_FFT_to_Hist<float, BinType, DataType>::compute_n_prod(
    np_double &np_array) {
...
TimeQuad_FFT_to_Hist<float, BinType, DataType>::copy_ks(np_double &np_ks,
                                                        uint n_prod) {
  ...
  size_t num_bytes =
      shape[0] * strides[0] / 2; // Number of bytes taken by the new array
...
template <class BinType, class DataType>
uint TimeQuad_FFT_to_Hist<float, BinType, DataType>::compute_l_kernels(
    np_double &np_array) {
  py::buffer_info buffer = np_array.request();
  std::vector<ssize_t> shape = buffer.shape;
...
template <class BinType, class DataType>
std::vector<ssize_t>
TimeQuad_FFT_to_Hist<float, BinType, DataType>::get_shape(np_double &np_array) {
  return np_array.request().shape;
}

template <class BinType, class DataType>
uint64_t TimeQuad_FFT_to_Hist<float, BinType, DataType>::compute_l_data(
...
template <class BinType, class DataType>
void TimeQuad_FFT_to_Hist<float, BinType, DataType>::prepare_kernels(
    np_double &np_ks) {
...
template <class BinType, class DataType>
void TimeQuad_FFT_to_Hist<float, BinType, DataType>::execution_checks(
...
template <class BinType, class DataType>
void TimeQuad_FFT_to_Hist<float, BinType, DataType>::execute_py(
...
template <class BinType, class DataType>
void TimeQuad_FFT_to_Hist<float, BinType, DataType>::execute(
    Multi_array<DataType, 1, uint64_t> &data) {
...
#pragma omp for
    for (uint i = 0; i < n_chunks; i++) {
      ...
      for (uint j = 0; j < n_prod; j++) {
        ACCUMULATE(this_thread, j, ((float *)hs(this_thread, j)) + l_qs_chunk,
                   l_fft - 2 * l_qs_chunk)
      }
      for (uint j = 0; j < n_prod; j++) {
...
#pragma omp single
    {
      if (l_reste != 0) {
        ...
        fftwf_execute_dft_c2r(
            h_plan, reinterpret_cast<fftwf_complex *>(hs(this_thread)),
            (float *)hs(this_thread));
        for (uint j = 0; j < n_prod; j++) {
          for (uint k = 0; k < l_kernel - 1; k++) {
            ((float *)hs(this_thread, j))[k] +=
                quads(j, (n_chunks)*l_qs_chunk + k);
          }
...
#pragma omp for simd
    for (uint j = 0; j < n_prod; j++) {
      ACCUMULATE(this_thread, j, quads(j) + l_qs_chunk,
                 (n_chunks - 1) * l_qs_chunk)
    }
    // reduction
    if (this_thread != 0) {
      for (uint j = 0; j < n_prod; j++) {
...
template <class BinType, class DataType>
inline void TimeQuad_FFT_to_Hist<float, BinType, DataType>::float_to_hist(
    float data, BinType *histogram, double max, double bin_width) {
  std::abs(data) >= max
      ? histogram[0]++
...
TimeQuad_FFT_to_Hist<float, BinType, DataType>::get_Histograms_py() {
...
template <class BinType, class DataType>
py::array_t<double>
TimeQuad_FFT_to_Hist<float, BinType, DataType>::abscisse_py(double max,
                                                            uint nofbins) {
