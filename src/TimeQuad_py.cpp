...
"\t Class_FloatType_DataType, where '_XType' behaves like Template Argument:\n"
...
"\t\t Similarly to generalized universal functions the algoritm is applied along the last axis of ks and repeated for all other dimensions.\n"
"\t data (numpy array) : .shape = (l_data,) and dtype = int16 \n"
"\t\t data's shape must de the same in the constructor and the TimeQuad_FFT.execute(data) method. \n"
...
"\t\t Memory is allocated once at construction and re-used, therefore the TimeQuad_FFT.execute(data) method, sets the number of threads at each call to ensure compatibility with memory layout. \n" ;

const char* s_TQtoH = 
"\t TimeQuad_FFTtoHist implements an fft convolutions followed by a deduction of the data into 1D-histograms, all parallelizedwith openmp\n"
"\t This uses a lot less memory compared to TimeQuad_FFT. Only the first l_kernel-1 and last l_kernel-1 for each fft_chunk is stored in ram. The saving in memory cost should roughly be l_kernel/l_fft. \n"
"\t Multiples kernels can be applied in parallel.\n"
"\t Class_FloatType_BinType_DataType, where '_XType' behaves like Template Argument:\n"
"\t\t FloatType is the typed used for FFTW (float or double)\n"
"\t\t BinType   is the typed used for all histograms bins (uint32_t or uint64_t) \n"
"\t\t DataType  is the typed used for the input data (only int16_t for now)\n"
"\t ks (numpy array): kernels with shape (..., l_kernel).\n"
"\t\t Similarly to generalized universal functions the algoritm is applied along the last axis of ks and repeated for all other dimensions.\n"
"\t data (numpy array) : .shape = (l_data,) and dtype = int16 \n"
"\t\t data's shape must de the same in the constructor and the TimeQuad_FFT.execute(data) method. \n"
...
#define PY_TIME_QUAD_FFT(FloatType,DataType)\
	py::class_<TimeQuad_FFT<FloatType,DataType>>( m , "TimeQuad_FFT_"#FloatType"_"#DataType , s_TQ)\
	.def\
	(\
...
#define PY_TIME_QUAD_FFT_TO_HIST(FloatType,BinType,DataType)\
	py::class_<TimeQuad_FFT_to_Hist<FloatType,BinType,DataType>>( m , "TimeQuad_FFT_"#FloatType"_to_Hist_"#BinType"_"#DataType , s_TQtoH)\
	.def\
	(\
