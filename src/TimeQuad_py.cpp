...
const char *s_TQ = "\t TimeQuad_FFT implements an FFT convolution algorithm parallelized "
                   "with OpenMP.\n"
                   ...
                   "\t\t DataType is the type used for the input data (currently only int16).\n"
                   ...
                   "\t\t Memory is allocated once during construction and re-used. Therefore, the "
                   "number of threads is set during each call to TimeQuad_FFT.execute(data) to "
                   "ensure compatibility with memory layout.\n"
                   "\t Methods:\n"
                   "\t\t .execute(ks, data): Takes in kernels (ks[..., l_kernel]) and data as numpy "
                   "array (e.g., int16), performs the convolution, and stores the result in the "
                   "'quad' attribute.\n"
                   "\t\t .quads(): Returns access to the result of the convolution.\n"
                   "\t\t .reset(): Resets the state of the object to allow reuse without "
                   "reinstantiating.\n"
                   "\t\t .abscisse(): Static method to obtain the abscissa (X-axis) array defining "
                   "the histogram.";

const char *s_TQtoH = "\t TimeQuad_FFTtoHist implements an FFT convolution followed by a "
                      ...
                      "Only the first l_kernel-1 and last l_kernel-1 of each FFT chunk are stored "
...
const char *s_TQtoH2D = "\t TimeQuad_FFTtoHist2D implements an FFT convolution followed by a "
                        ...
                        "\t n_threads (uint): Number of OpenMP threads to be used.\n"
                        "\t\t Memory is allocated once during construction and re-used. Therefore, the "
                        "number of threads is set during each call to TimeQuad_FFTtoHist2D.execute(data) "
                        ...
                        "\t\t .reset(): Resets the state of the object to allow reuse without "
                        "reinstantiating.\n"
                        "\t\t .abscisse(): Static method to obtain the abscissa (X and Y-axis) arrays "
...
#define PY_TIME_QUAD_FFT(FloatType, DataType)                                                      \
    py::class_<TimeQuad_FFT<FloatType, DataType>>(m, "TimeQuad_FFT_" #FloatType "_" #DataType,     \
                                                  s_TQ)                                            \
        .def(py::init<np_double, np_int16, double, uint, int>(), "ks"_a.noconvert(),               \
...
#define PY_TIME_QUAD_FFT_TO_HIST2D(FloatType, BinType, DataType)                                     \
    py::class_<TimeQuad_FFT_to_Hist2D<FloatType, BinType, DataType>>(                                \
        m, "TimeQuad_FFT_" #FloatType "_to_Hist2D_" #BinType "_" #DataType, s_TQtoH2D)                 \
        .def(py::init<np_double, np_int16, double, uint, uint, double, int>(), "ks"_a.noconvert(), \
             "data"_a.noconvert(), "dt"_a.noconvert(), "l_fft"_a.noconvert(),                      \
             "nb_of_bins"_a.noconvert(), "max"_a.noconvert(), "n_threads"_a.noconvert())           \
